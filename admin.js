...
import { getDatabase, ref, onValue, push, set, update, remove } from "https://www.gstatic.com/firebasejs/9.22.0/firebase-database.js";
...
addEditProductBtn.addEventListener('click', async () => {
    ...
    try {
        if (id) {
            ...
            await set(ref(database, 'products/' + id), { id, ...productData });
            alert('Product updated successfully!');
        } else {
            // Add new product
            const newProductRef = push(ref(database, 'products'));
            const newProductId = newProductRef.key;
            await set(newProductRef, { id: newProductId, ...productData });
...
async function updateOrderStatus(orderKey, newStatus) {
    try {
        await update(ref(database, 'orders/' + orderKey), { status: newStatus });
